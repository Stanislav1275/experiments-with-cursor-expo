...
// Синий фон активного таба — полоска анимированно переползает
...
const MediaGridItem = ({ item }: { item: { id: string } }) => (
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (containerRef.current) {
        const current = containerRef.current.getCurrentIndex();
        const target = TAB_NAMES.indexOf(initialTab);
        if (current !== target) containerRef.current.jumpToTab(initialTab);
      }
    }, 100);
  ...
  const renderTabBar = useCallback(
    (props: TabBarProps<TabName>) => (
      <TabBarWithIndicator props={props} insets={insets} />
    ),
    [insets]
  );
  ...
  return (
    <View style={styles.container}>
      <Tabs.Container
        ...
        onTabChange={({ tabName }) =>
          handleTabChange({ tabName: tabName as TabName })
        }
        minHeaderHeight={0}
